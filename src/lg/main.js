				...
				ready: function () {
					...
					if (this.readyIndex == this.img.length) {
						...
						this.frame = setTimeout(() => {
							...
							for (var i = 0; i < this.img.length; i++)
								root.refs['v' + i].play(this.sortIndex);
						}, this.delay);
					}
				},
				flapAll: function () {
			...
			for (var i = 0; i < 18; i++) {
				op.push(
					<Video
						...
						key={i}
						index={i}
						re={this.videoRe.bind(this)}
						clicked={this.videoSelected.bind(this)}
						ready={this.videoReady.bind(this)}
					/>
				);
			}
			return op;
		...
		return (
			<div id='lg'>
				<div class='containers'>
					...
					<Mask
						ref='mask'
						out={this.maskOut.bind(this)}
						click={this.maskClick.bind(this)}
					/>
				</div>
			</div>
		);
